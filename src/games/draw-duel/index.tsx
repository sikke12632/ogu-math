...
export function resolveTeamScores(results: MatchResult[], teams: Team[]): TeamScore[] {
  ...
  for (const r of results) {
    ...
    if (tid) wins.set(tid, (wins.get(tid) ?? 0) + 1)
  }
  return teams.map((t) => ({ teamId: t.id, wins: wins.get(t.id) ?? 0 }))
}

/** 보드 뷰에 뜨는 팀 승수. **개인 전적은 절대 그리지 않는다** */
function DrawDuelBoard({ teams, scores, round }: GameBoardProps) {
  const max = Math.max(1, ...scores.map((s) => s.wins))
  return (
          ...
          return (
            <li key={t.id}>
              ...
              <span className="board-bar">
                <span className="board-bar-fill" style={{ width: `${(wins / max) * 100}%` }} />
              </span>
              <span className="board-team-wins">{wins}승</span>
            </li>
          )
        })}
      </ul>
...
export const drawDuel: GameModule = {
  id: 'draw-duel',
  name: '번호 뽑기 대전',
  tagline: '통을 흔들어 번호 공을 뽑는다. 꽝을 뽑으면 그 자리에서 패배.',
  grouping: 'duel',
  matchSize: 2,
  rounds: 3,
